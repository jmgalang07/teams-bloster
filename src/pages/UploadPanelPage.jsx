...
const fallbackWaterTypeOptions = ['Embalse', 'Charca', 'Rio', 'Canal', 'Lago', 'Pais / zona'];
const fallbackWaterDifficultyOptions = ['Baja', 'Baja / media', 'Media', 'Media / alta', 'Alta'];
...
export default function UploadPanelPage() {
  const {
    ...
    rigOptions,
    fishTypes,
    waterTypeOptions: storedWaterTypeOptions,
    waterDifficultyOptions: storedWaterDifficultyOptions,
    lastError,
    addCapture,
    updateCapture,
    deleteCapture,
  ...
  const waterTypeOptions = storedWaterTypeOptions?.length ? storedWaterTypeOptions : fallbackWaterTypeOptions;
  const waterDifficultyOptions = storedWaterDifficultyOptions?.length
    ? storedWaterDifficultyOptions
    : fallbackWaterDifficultyOptions;
  const fishTypeOptions = fishTypes?.length ? fishTypes : [
  ...
  const handleCaptureSubmit = async (event) => {
    ...
    try {
      ...
      const savedCapture = editingCaptureId
        ? await updateCapture(editingCaptureId, payload)
  ...
  const handleWaterSubmit = async (event) => {
    ...
    try {
      const payload = {
        ...
        image: waterImage || (editingWaterId ? waters.find((item) => item.id === editingWaterId)?.image : 'storage://team-assets/storage://team-assets/images/logo.png'),
      };
  ...
  const handleDeleteWater = async (water) => {
  ...
  const handleExport = () => {
    ...
    setManagementStatus({
      tone: 'success',
      text: 'Respaldo exportado correctamente con las altas guardadas en Supabase.',
    });
  };

  const handleProjectSyncExport = () => {
    setManagementStatus(null);
    downloadJsonFile(projectSyncPayload, `teams-bloster-supabase-export-${today}.json`);

    setManagementStatus({
      tone: 'success',
      text: 'Exportacion JSON creada desde los datos actuales de Supabase.',
    });
  };
  ...
  const handleImportBackup = async (event) => {
    ...
    try {
      ...
      const result = await importCustomData(parsed);
  ...
  const handleReset = async () => {
    const confirmed = window.confirm(
      'Se borraran las capturas y escenarios creados como altas en Supabase. ¿Quieres continuar?',
    );
    ...
    setManagementStatus({
      tone: 'success',
      text: 'Se han borrado las altas marcadas como custom en Supabase.',
    });
  };
        ...
        <section className="section section-alt">
          <div className="site-container">
            <div className="status-message status-error">
              {lastError.message || 'No se ha podido conectar con Supabase.'}
            </div>
          </div>
        </section>
      ) : null}
          ...
          <div className="management-grid">
            ...
            <article className="info-card management-card">
              <div className="form-card-heading">
                ...
                <p>
                  Los datos visibles se guardan ahora directamente en Supabase. El JSON queda solo como respaldo manual si quieres conservar una copia.
                </p>
                <p>
                  Estado de Supabase: <strong>{projectSyncStatus === 'ready' ? 'conectado' : projectSyncStatus === 'loading' ? 'cargando' : 'revisar variables'}</strong>.
                </p>
              </div>
